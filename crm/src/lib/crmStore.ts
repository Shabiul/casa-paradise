...
export async function pullLatestFromSupabase(force = false): Promise<boolean> {
  ...
  const now = Date.now();
  // Throttle rapid repeated calls unless forced
  if (!force && now - lastPullTimestamp < 1500) {
    return true;
  }
  // Deduplicate overlapping in-flight calls
...
function initSingletonRealtimeSync() {
  ...
  if (isSupabaseConfigured()) {
    setupSupabaseRealtimeChannel(() => {
      // Debounce rapid postgres changes into a single consolidated fetch
...
// Auto-initialize once on client load
if (typeof window !== 'undefined') {
...
export function updateStaffUser(userId: string, updates: Partial<CRMUser>): CRMUser | null {
  ...
  const isPromotedToAdmin = updates.role === 'admin';
  const isDemotedToStaff = updates.role === 'staff' && current.role === 'admin';

  const updatedPermissions = isPromotedToAdmin
    ? { ...defaultAdminPermissions }
    : isDemotedToStaff
      ? { ...defaultStaffPermissions, ...(updates.permissions || {}) }
      : {
          ...current.permissions,
          ...(updates.permissions || {})
        };
...
export function deleteStaffUser(userId: string): boolean {
  ...
  const isCurrentActive = store.activeUserId === userId || (typeof window !== 'undefined' && localStorage.getItem(ACTIVE_USER_STORAGE_KEY) === userId);
  if (isCurrentActive) {
    const fallback = store.users.find(u => u.role === 'admin') || store.users[0];
    const newActiveId = fallback ? fallback.id : 'USR-ADMIN-1';
    store.activeUserId = newActiveId;
    if (typeof window !== 'undefined') {
      localStorage.setItem(ACTIVE_USER_STORAGE_KEY, newActiveId);
    }
  }
